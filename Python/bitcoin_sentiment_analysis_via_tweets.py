# -*- coding: utf-8 -*-
"""Bitcoin_Sentiment_Analysis_via_Tweets.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1M5yZNheA8sYUX1P8yYQLCz3b3qMl1KB3
...
- After text cleaning and sentiment analysis, the initial dataset shows a higher number of tweets with positive (32,005) and neutral (33,763) sentiment than those with negative sentiment (9,010).

- As shown in the line graph, sentiment variations peaked for all types of sentiment on 10 May 2019.

- Despite the different numbers of positive and negative tweets, the distributions of likes and replies are highly skewed to the right and dominated by values equal to zero. As a result, the median is zero in both sentiment classes. Furthermore, the density graphs (KDE) and boxplots show a marked overlap between the two distributions.

  Considering the descriptive and graphical evidence, no inferential statistical test (e.g. Mann–Whitney or Kolmogorov–Smirnov) was applied, as the analysis carried out is sufficient to answer the initial question regarding the differences in the number of likes and replies between positive and negative tweets. Furthermore, the following dataset includes both users who wrote positive and negative tweets (violation of independence assumption) and an asymmetric number of users, which could affect the test results.



- Finally, although the line graph showing the variation in sentiment of tweets about Bitcoin and the variation in the daily price of Bitcoin appears to have “local co-variations”, in this dataset they are not associated, as shown by the correlation matrix (*r= -0.042*) and the regression graph, which identify an almost zero and slightly negative association.
"""